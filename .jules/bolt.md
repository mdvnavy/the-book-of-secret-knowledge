...
## 2024-05-15 - pkill Optimization
**Learning:** Replacing a loop of `kill` commands based on chained process inspections (`ps afx | grep | grep | awk`) with a single `pkill -f` command natively improves performance by drastically reducing process forks and executing in a fraction of the time.
**Action:** Use `pkill` (or `pgrep`) instead of subshells and chained grep/awk pipelines to match and terminate processes.
