...
## 2024-06-25 - Regex Extraction vs AWK and xargs vs while loop
**Learning:** In the snippet `netstat -nlt | grep 'tcp ' | grep -Eo "[1-9][0-9]*" | xargs -I {} sh -c ...`, the regex `grep -Eo "[1-9][0-9]*"` is inherently buggy as it erroneously extracts IP address octets (e.g., `127` and `53` from `127.0.0.53:53`) in addition to the actual port number, leading to failed `nc` attempts. Furthermore, piping into `xargs -I {} sh -c` forces a subshell and binary spawn for every match, suffering O(N) process overhead.
**Action:** Replace fragile `grep -Eo` extractions on tabular data with precise `awk` field parsing, capturing the exact substring using `split()` (e.g. `awk '/tcp / {n=split($4, a, ":"); print a[n]}'`). To optimize execution, replace `xargs -I {} sh -c "..."` with a native bash `| while read -r port; do ... done` loop, completely eliminating the subshell overhead for each line.
