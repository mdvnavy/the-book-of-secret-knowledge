...
## 2026-06-30 - [Optimize Process Inspections]
**Learning:** Chained process inspections like `ps auxw | grep pattern | awk` spawn multiple unnecessary subprocesses and have high overhead.
**Action:** Use native `pgrep -f pattern` combined with `awk '{print $1}'` to eliminate the `ps` and `grep` process forks, improving shell script performance.
