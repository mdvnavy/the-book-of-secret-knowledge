...
## 2024-05-28 - Replace Process Inspections with Native pkill
**Learning:** Using chained process inspections in shell scripts (e.g., `ps afx | grep | grep | awk` inside a loop) spawns multiple subprocesses per invocation, creating unnecessary overhead. A native `pkill -u` command can achieve the same result without process forks.
**Action:** When optimizing shell scripts, replace complex loops that inspect processes with native commands like `pkill` to eliminate process forks and subshell overhead.
